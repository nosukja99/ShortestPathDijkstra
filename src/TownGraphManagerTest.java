...
public class TownGraphManagerTest {
	...
	public void testDeleteRoadConnection() {
		...
		graph.containsRoadConnection(town[2], town[11]);
	...
	public void testGetPathSTUDENT() {
		ArrayList<String> path = graph.getPath(town[5],town[2]);
		  assertNotNull(path);
		  assertTrue(path.size() > 0);
		  assertEquals("Town_5 via Road_3 to Town_1 6 mi",path.get(0).trim());
    	  assertEquals("Town_1 via Road_1 to Town_2 2 mi",path.get(1).trim());
	}
